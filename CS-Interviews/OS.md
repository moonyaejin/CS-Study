...
> 출처: 개발자: 데일리 CS 역량 강화 챌린지 (2025.10.24/2026.01.06)

**답변**

뮤텍스와 세마포어는 둘 다 멀티스레드 환경에서 동기화를 위한 기법인데, 가장 큰 차이는 동시에 접근할 수 있는 스레드 수와 소유권입니다.

뮤텍스는 오직 하나의 스레드만 접근할 수 있습니다. 화장실 열쇠처럼 한 사람이 열쇠를 가지고 들어가면 다른 사람은 기다려야 합니다. 그리고 열쇠를 가진 사람만 문을 열 수 있는 것처럼, 잠금을 획득한 스레드만 해제할 수 있습니다.

세마포어는 정해진 개수만큼 동시에 접근할 수 있습니다. 주차장을 생각하면 되는데, 10자리 주차장이면 10대까지 들어올 수 있고 한 대가 나가면 한 대가 들어올 수 있습니다. 그리고 뮤텍스와 달리 소유권이 없어서, wait을 호출한 스레드와 signal을 호출하는 스레드가 달라도 됩니다.

정리하면 뮤텍스는 하나의 자원을 보호할 때, 세마포어는 여러 개의 자원을 관리하거나 스레드 간 신호를 주고받을 때 사용합니다.
