...
데이터베이스는 여러 사용자가 동시에 접근하는데, 아무런 제한 없이 서로의 작업을 보게 되면 문제가 생깁니다. 예를 들어 A가 계좌에서 돈을 빼는 중인데, B가 그 중간 상태의 잔액을 읽어버리면 잘못된 정보를 보게 되죠. 격리 수준은 이런 **동시성 문제와 데이터 정합성 사이의 균형**을 조절합니다.
