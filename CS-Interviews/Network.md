...
15. [HTTP와 HTTPS의 차이점은 무엇인가요R?](#15-http와-https의-차이점은-무엇인가요)
...
> 출처: 개발자: 데일리 CS 역량 강화 챌린지 (2025.11.05/2026.01.19)
...
> 출처: 개발자: 데일리 CS 역량 강화 챌린지 (2025.11.17/2026.01.30)

**답변**

CORS는 서로 다른 출처 간에 리소스를 공유할 수 있도록 허용하는 메커니즘입니다. 여기서 출처는 도메인, 포트, 프로토콜을 말합니다.

브라우저는 보안을 위해 기본적으로 다른 출처의 요청을 막습니다. 악의적인 사이트가 다른 사이트 데이터를 가져가는 걸 방지하기 위함입니다. 근데 실제 개발할 때는 프론트엔드가 localhost:3000이고 백엔드가 localhost:8080처럼 포트가 다른 경우가 많습니다. 이 때 서버가 CORS 허용 헤더를 안 보내면 브라우저가 응답을 차단해서 에러가 납니다.

서버는 정상 응답을 보냈는데 브라우저가 막는것이기 때문에 이를 해결하기 위해서는 서버에 Access-Control-Allow-Origin 헤더로 허용할 출처를 설정해줘야 합니다.

## **예상 꼬리질문**

**Q1. 왜 Postman이나 curl에서는 CORS 에러가 안 나나요?**
...
**Q2. Spring에서 CORS는 어떻게 설정하나요?**
A. 컨트롤러에 @CrossOrigin 어노테이션을 붙이거나, WebMvcConfigurer로 전역 설정하는 방법이 있습니다.

**Q3. Preflight 요청이 뭔가요?**
A. 브라우저가 본 요청 전에 "이거 보내도 돼?"라고 먼저 확인하는 OPTIONS 요청입니다. PUT, DELETE 같은 요청이나 커스텀 헤더가 있으면 Preflight가 먼저 발생하고, 서버가 허용 응답을 보내야 본 요청이 진행됩니다.

**Q4. 개발 중에 CORS 에러 나면 어떻게 하나요?**
A. 프론트엔드 개발 서버에서 프록시 설정을 하면 됩니다. 브라우저 입장에서는 같은 출처로 보여서 CORS가 안 생기기 때문입니다. 근데 이건 개발용이고, 배포할 때는 서버에서 제대로 설정해야 합니다.
...
> 출처: 개발자: 데일리 CS 역량 강화 챌린지 (2025.11.21/2026.02.05)

**답변**

API Gateway는 클라이언트와 백엔드 서비스 사이에서 모든 API 요청을 받아서 적절한 서비스로 전달해주는 단일 진입점입니다.
마이크로서비스 환경에서는 상품, 주문, 결제, 회원 서비스가 각각 다른 서버에서 돌아갑니다. 클라이언트가 이 주소들을 다 알고 각각 호출하면 복잡하기 때문에 API Gateway로 요청을 받아 경로에 따라 알맞은 서비스로 라우팅해줍니다.
라우팅 외에도 인증, 로깅, 속도 제한 같은 공통 기능을 Gateway에서 한 번에 처리할 수 있어서, 각 서비스마다 따로 구현할 필요가 없습니다. 그리고 내부 서비스를 외부에 직접 노출하지 않아서 보안에도 좋습니다.

## **예상 꼬리질문**

**Q1. API Gateway의 단점은 무엇인가요?**

A. 모든 요청이 Gateway를 거치기 때문에 Gateway가 죽으면 전체 시스템이 마비되는 단일 장애점이 될 수 있습니다.
그래서 보통 Gateway를 여러 대 두고 앞에 로드밸런서를 붙여서 가용성을 확보합니다.

또 모든 트래픽이 Gateway를 거쳐가기 때문에 병목이 될 수 있고, 한 단계가 추가되므로 약간의 지연이 발생할 수도 있습니다.

**Q2. Load Balancer는 무엇이며, API Gateway와 어떤 차이가 있나요?**

A. Load Balancer는 들어오는 트래픽을 여러 서버에 균등하게 분산시켜주는 장치입니다.
서버 한 대로 감당이 안 되는 경우 여러 대를 두고 요청을 나눠서 처리하게 해 줍니다.

예를 들어 주문 서비스가 트래픽을 감당하지 못해서 동일한 주문 서비스 서버를 3대로 늘렸다고 하면, Load Balancer가 앞에서 요청을 받아서 서버 A, B, C에 순서대로 또는 부하 상태에 따라 분배합니다. 사용자는 하나의 주소로 요청하지만 실제로는 여러 서버가 나눠서 처리하는 것입니다.

Load Balancer는 같은 서비스의 여러 인스턴스에 분산하는 게 주 목적인 반면, API Gateway는 다양한 서비스로 라우팅하면서 인증이나 속도 제한 같은 기능도 처리한다는 점입니다.

실제로 둘을 같은 쓰는 경우가 많고, Gateway 앞에 Load Balancer를 둬서 Gateway 자체의 가용성을 높이는 구조입니다.
