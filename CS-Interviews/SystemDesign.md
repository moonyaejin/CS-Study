...
3. [마이크로서비스 아키텍처(Microservices Architecture)란 무엇이고, 모놀리식 아키텍처와 어떤 차이가 있나요?](#3-마이크로서비스-아키텍처microservices-architecture가-무엇이고-모놀리식-아키텍처와-어떤-차이가-있나요)
...
## 3. 마이크로서비스 아키텍처(Microservices Architecture)가 무엇이고, 모놀리식 아키텍처와 어떤 차이가 있나요?
> 출처: 개발자: 데일리 CS 역량 강화 챌린지 (2025.12.10)

**답변**

**마이크로서비스 아키텍처**는 애플리케이션을 독립적인 작은 서비스들로 분리한 구조입니다. 각 서비스는 자체 데이터베이스를 가지고, 독립적으로 배포되며, API를 통해 서로 통신합니다.

**모놀리식 아키텍처**는 하나의 애플리케이션에 모든 기능이 통합된 구조입니다. 하나의 코드베이스, 하나의 배포 단위로 운영됩니다.

| 구분 | 모놀리식 | 마이크로서비스 |
|------|---------|---------------|
| 배포 | 전체를 한 번에 배포 | 서비스별 독립 배포 |
| 확장 | 전체 애플리케이션 확장 | 필요한 서비스만 확장 |
| 기술 스택 | 단일 기술 스택 | 서비스별 다른 기술 가능 |
| 장애 영향 | 한 부분 장애가 전체에 영향 | 장애가 해당 서비스에 격리 |
| 복잡도 | 초기 개발이 단순 | 분산 시스템 복잡도 증가 |


## **예상 꼬리질문**

**Q1. 마이크로서비스의 장점과 단점은 무엇인가요?**

A. **장점**
- **독립적 배포**: 한 서비스 수정이 다른 서비스에 영향 없음. 빠른 배포 주기 가능
- **기술 유연성**: 서비스별로 최적의 언어, 프레임워크 선택 가능
- **확장성**: 트래픽이 많은 서비스만 선택적으로 Scale Out
- **장애 격리**: 한 서비스 장애가 전체 시스템을 멈추지 않음

**단점**
- **분산 시스템 복잡도**: 네트워크 통신, 데이터 일관성, 장애 추적이 어려움
- **운영 부담**: 서비스마다 배포, 모니터링, 로깅 필요
- **트랜잭션 관리 어려움**: 여러 서비스에 걸친 트랜잭션 처리가 복잡


**Q2. 마이크로서비스 간 통신 방식에는 어떤 것들이 있나요?**

A. 크게 **동기 방식**과 **비동기 방식**이 있습니다.

**동기 방식 (Synchronous)**
- REST API: 가장 일반적. HTTP 기반으로 단순하고 범용적
- gRPC: 바이너리 프로토콜로 REST보다 빠름. 서비스 간 내부 통신에 적합

**비동기 방식 (Asynchronous)**
- 메시지 큐 (Kafka, RabbitMQ): 서비스 간 직접 의존 없이 이벤트 기반 통신
- 느슨한 결합으로 장애 전파 방지, 트래픽 버퍼링 가능

실시간 응답이 필요하면 동기 방식, 느슨한 결합과 안정성이 중요하면 비동기 방식을 선택합니다.


**Q3. 모놀리식에서 마이크로서비스로 전환할 때 주의할 점은 무엇인가요?**

A. **점진적으로 전환해야 합니다.** 한 번에 전체를 분리하면 위험합니다.

먼저 모놀리식 내에서 모듈 경계를 명확히 나누고, 가장 독립적이거나 변경이 잦은 기능부터 하나씩 분리합니다. 이를 **Strangler Fig 패턴**이라고 합니다. 새 기능은 마이크로서비스로 만들고, 기존 기능을 점진적으로 마이그레이션하는 방식입니다.

또한 처음부터 마이크로서비스로 시작하는 것도 주의해야 합니다. 도메인 이해가 부족한 초기에는 서비스 경계를 잘못 나눌 위험이 크고, 작은 팀에서는 운영 부담이 오히려 커질 수 있습니다.


**Q4. 마이크로서비스에서 데이터 일관성은 어떻게 유지하나요?**

A. 각 서비스가 자체 DB를 가지므로 기존 ACID 트랜잭션을 사용할 수 없습니다. 대신 **최종 일관성(Eventual Consistency)**을 받아들이고, 다음 패턴들을 활용합니다.

- **Saga 패턴**: 각 서비스의 로컬 트랜잭션을 순차적으로 실행하고, 실패 시 보상 트랜잭션으로 롤백
- **이벤트 소싱**: 상태 변경을 이벤트로 기록하고, 다른 서비스가 이벤트를 구독해 동기화

예를 들어 주문 서비스에서 주문 생성 → 재고 서비스에서 재고 차감 → 결제 서비스에서 결제 처리를 순차 실행하고, 결제 실패 시 재고 복구 → 주문 취소로 보상합니다.
